...
\begin{align}
\nabla J(\theta, \phi) &= \mathbb{E}_{p_{\theta,\phi}(x_{1:T}, z_{1:T})}\left[R(x_{1:T}) \nabla \log p_{\theta,\phi}(x_{1:T}, z_{1:T})\right] \\
&= \mathbb{E}_{p_{\theta,\phi}(x_{1:T}, z_{1:T})}\left[R(x_{1:T}) \sum_{t=1}^T \nabla \log p_{\theta,\phi}(x_t, z_t | x_{<t})\right] \\
&= \mathbb{E}_{p_{\theta,\phi}(x_{1:T}, z_{1:T})}\left[R(x_{1:T}) \sum_{t=1}^T \left(\nabla_\theta \log p_\theta(x_t|z_t, x_{<t}) + \nabla_\phi \log p_\phi(z_t|x_{<t})\right)\right]
\end{align}

With a baseline $b(x_{<T})$, we can write this as:
\begin{align}
\nabla J(\theta, \phi) &= \mathbb{E}_{p_{\theta,\phi}(x_{1:T}, z_{1:T})}\left[A(x_{1:T}) \sum_{t=1}^T \left(\nabla_\theta \log p_\theta(x_t|z_t, x_{<t}) + \nabla_\phi \log p_\phi(z_t|x_{<t})\right)\right]
\end{align}
where $A(x_{1:T}) = R(x_{1:T}) - b(x_{<T})$.

\subsection{Separating Token and Router Gradients}
The gradient with respect to token generation parameters $\theta$ is:
\begin{align}
\nabla_\theta J(\theta, \phi) &= \mathbb{E}_{p_{\theta,\phi}(x_{1:T}, z_{1:T})}\left[A(x_{1:T}) \sum_{t=1}^T \nabla_\theta \log p_\theta(x_t|z_t, x_{<t})\right]
\end{align}

The gradient with respect to router parameters $\phi$ is:
\begin{align}
\nabla_\phi J(\theta, \phi) &= \mathbb{E}_{p_{\theta,\phi}(x_{1:T}, z_{1:T})}\left[A(x_{1:T}) \sum_{t=1}^T \nabla_\phi \log p_\phi(z_t|x_{<t})\right]
\end{align}

Note that both gradients require sampling from the full joint distribution $p_{\theta,\phi}(x_{1:T}, z_{1:T})$, which means we must sample both tokens $x_{1:T}$ and expert choices $z_{1:T}$ together during rollout.
...
When sampling trajectories from $p_{\text{sample}}(x_{1:T}, z_{1:T})$ but training under $p_{\text{train}}(x_{1:T}, z_{1:T})$, we require importance sampling:
\begin{align}
\nabla J_{\text{train}} &= \mathbb{E}_{p_{\text{sample}}(x_{1:T}, z_{1:T})}\left[\frac{p_{\text{train}}(x_{1:T}, z_{1:T})}{p_{\text{sample}}(x_{1:T}, z_{1:T})} A(x_{1:T}) \sum_{t=1}^T \nabla \log p_{\text{train}}(x_t, z_t|x_{<t})\right]
\end{align}

The importance weight decomposes as:
$$\frac{p_{\text{train}}(x_{1:T}, z_{1:T})}{p_{\text{sample}}(x_{1:T}, z_{1:T})} = \prod_{t=1}^T \frac{p_{\text{train}}(x_t|z_t, x_{<t})}{p_{\text{sample}}(x_t|z_t, x_{<t})} \cdot \frac{p_{\text{train}}(z_t|x_{<t})}{p_{\text{sample}}(z_t|x_{<t})}$$

This importance weight is critical for maintaining unbiased gradient estimates when the sampling and training distributions differ. \textbf{The key insight is that we must replay the expert choices $z_{1:T}$ from the sampling distribution to correctly estimate gradients for the router.}
