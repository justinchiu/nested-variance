...
This importance weight is critical for maintaining unbiased gradient estimates when the sampling and training distributions differ. \textbf{The key insight is that we must replay the expert choices $z_{1:T}$ from the sampling distribution to correctly estimate gradients for the router.} Following standard practice in policy optimization, we clip the importance weights to reduce variance; importantly, this clipping must be applied to the router weights as well as the token generation weights.
